...
关于wordpress的插件定义
什么是wp插件呢？plugins目录下的插件和主题中的functions文件中的函数有什么不同呢？
【它们两者引入的方式一样，但位置不同最后决定了它们不同的意义】
学习之初会有这个疑问，因为我们在插件里写一些代码和在functions写的代码执行起来完全一样！
假如我切换了一个主题时，functions文件也会跟着切换，这就functions只服务于各自的主题
假如我启用了一个插件，那么在这里系统中的所有内容都会受到这个插件的作用
以上就是两者在作用范围上存在的差异！
当前我想开发一个CMS系统，里面包含了一切修改主题的功能，包括对默认系统功能的整理，新功能的添加等等
它没有生成一个全新的功能，而且他的作用与functions基本一样，只是组合比较多而已
它应该算得上是一个主题文件吧，
事实并不是这样，修改比较鲜明的插件通常是以一个全新的功能出发的，比如：电子邮件插件，表单插件，代码加亮插件等等
但是我开发一个CMS系统是一个插件，它的作用就是将wp改成一个CMS，最后再为这个系统开发相应的cms模板而且这个系统依赖于很多插件
因此我在学习过程中修改的那些零散的功能块只针对当前主题的一些测试，所以它们只能是functions
而且当他们有组织的组合成一个具有cms功能的整体时，他就应该是一个插件了，而且应该有一些完善它的配置参数可以修配
