...
从WordPress入口(大部分情况下是index.php)进入，到这部分结束为止，我们看到WordPress主要做了如下几件事情：定义常量和全局变量；
设置环境参数；
...
WordPress的初始化阶段是一个相当繁琐的过程，详细见如下列表：
    ...
    设置 PHP5 的默认时区。
    调用 wp_unregister_GLOBALS() 函数关闭 register_globals。
  unset 全局变量 $wp_filter, $cache_lastcommentmodified, $cache_lastpostdate，以确保他们不会存在。
    调用函数 wp_fix_server_vars() 规范 $_SERVER 变量设置。
  ...
  WP_CONTENT_DIR/advanced-cache.php，属于drop-in，供高级的缓存插件使用。
    ...
    加载 WP 的*大头（most of WP）*：
        ...
        WPINC/rewrite.php，定义一组 WP 的重写 API 以及 WP_Rewrite 类，用于重写链接的格式；
        并且该文件还定义了一组常量 EP_NONE，EP_PERMALINK，EP_ATTACHMENT，EP_DATE，EP_YEAR，EP_MONTH，EP_DAY，EP_ROOT，EP_COMMENTS，EP_SEARCH，EP_CATEGORIES，EP_TAGS，EP_AUTHORS，EP_PAGES，EP_ALL；
